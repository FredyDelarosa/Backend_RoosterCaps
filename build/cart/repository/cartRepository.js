    ...
    static getOrCreateShoppingCart(cliente_id) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const [existingCart] = yield (0, database_1.query)(checkCartSql, [cliente_id]);
                if (existingCart.length > 0) {
                    ...
                    const [result] = yield (0, database_1.query)(createCartSql, [cliente_id]);
                    if (!result.insertId) {
    ...
    static addCartDetail(carrito_id, gorra_id, tipo_gorra_id, talla_id, cantidad, status_apartado) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const [inventory] = yield (0, database_1.query)(checkInventorySql, [gorra_id, talla_id]);
                ...
                const [result] = yield (0, database_1.query)(createCartDetailSql, params);
                if (!result.insertId) {
    ...
    static getAllCartDetails() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const [cartDetails] = yield (0, database_1.query)(sql);
                if (!cartDetails.length) {
                    throw new Error('No cart details found');
                }
                const result = [];
                for (const detail of cartDetails) {
                    const [cap] = yield (0, database_1.query)('SELECT id, name, price, imagen FROM caps WHERE id = ?', [detail.gorra_id]);
                    if (!cap.length) {
                        throw new Error(`No cap found with id ${detail.gorra_id}`);
                    }
                    const [typeCap] = yield (0, database_1.query)('SELECT id, tipo FROM TypeCap WHERE id = ?', [detail.tipo_gorra_id]);
                    if (!typeCap.length) {
                        throw new Error(`No typeCap found with id ${detail.tipo_gorra_id}`);
                    }
                    const [size] = yield (0, database_1.query)('SELECT id, description FROM Size WHERE id = ?', [detail.talla_id]);
                    if (!size.length) {
